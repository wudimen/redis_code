...
void freeObjAsync(robj *key, robj *obj, int dbid) {				// 如果这个对呢内存过大的话，尝试去释放这个对象（添加删除任务，后台删除）
    ...
    if (free_effort > LAZYFREE_THRESHOLD && obj->refcount == 1) {
        atomicIncr(lazyfree_objects,1);			// 待删除数加一
        bioCreateLazyFreeJob(lazyfreeFreeObject,1,obj);			// 向bio_jobs尾部插入一个obj的删除任务
    } else {
        decrRefCount(obj);
    }
}
